...
def parseFromData(body, headers):
    ...
    parameters_dict["load_balancing_method"] = body.get(
        "load_balancing_method", "round-robin").lower()
    parameters_dict["health_monitor"] = body.get(
        "health_monitor", "tcp").lower()
    parameters_dict["pool_name"] = body.get("pool_name", "").lower()
    parameters_dict["members"] = body.get("members", "").lower()
    parameters_dict["f5_ips"] = body.get("f5_ips", "").lower()
    parameters_dict["vip_name"] = body.get("vip_name", "").lower()
    parameters_dict["ticket_number"] = body.get("ticket_number", "").lower()
...
def execute(event):
    ...
    try:
        ...
        client = Pools(method, body, headers, queryparams,
                       parsedParameters).operation()
        client.run()

        if client.code == 200:
            status = 'SUCCESS'
            message = "POOL WAS CREATED SUCCESSFULLY."
            code = client.code
